...
Foreign bar split by parent (top 8 by value + CHN forced, Other, hatched = unknown parent),
domestic in gray, bold total = MNE share, exactly the document's layout.
...
Region × region (value, row %, col %), top-15 parents × top-15 destinations (same three),
