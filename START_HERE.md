...
| 2a | `docs/WP_total_2026-09-23.pdf`, `docs/WP_sectors_2026-09-23.pdf`, `docs/WP_countries_2026-09-23.pdf` (the 09-21 versions are the old format) | the three reference documents (all goods / four sectors / nine origins) with EVERY figure, table and regression, now in the WP_draft2 format (title-case captions, short notes with a separate Source line, relabelled fragments), the numbers behind each figure next to it, no appendix; they are the place to look for relationships not used in the paper; rev. 6 adds the destination side of Fact 3 (who carries exports to the USA / China / EU-27; what US MNEs ship home) and the cross-sector summaries; Overleaf sources + zips in `output/wp/overleaf_WP_{total,sectors,countries}*`; rebuilt by `stylized_facts/python/wp_build_overleaf.py` |
...
| 2a'' | `docs/WP_draft_2026-09-21.pdf` | **the working-paper draft for Christian** (rev. 7 after his review): every caption tagged `[Main text]` / `[Appendix]`, dollar values as annual averages, no values on figures, the new exhibits he asked for; Overleaf source in `output/wp/overleaf_WP_draft*`; `python wp_build_overleaf.py draft` |
| 2a'' (v2) | `docs/WP_draft2_2026-09-23.pdf` | **second version of the exhibit draft** (29 pp): "Main Text" and "Appendix" as two separate parts; Tables 8, 16, 24 (agro SITC), 25, 26, 27 and Figure 7 (OECD split) of the 09-21 draft moved to the appendix; Notes and Source on separate lines, each note opening with a plain description; figure-internal labels (axes, legends, panel titles) rewritten in the plotting scripts and figures regenerated; referee-facing titles; every note replaced by a one- or two-sentence note ending in a *Source:* line; internal labels inside the fragments relabelled for this document only (`D2_RELABEL`). `python wp_build_overleaf.py draft2` |
| 2a''' (latest) | `docs/WP_paper_2026-09-23.pdf`, `docs/WP_paperagro_2026-09-23.pdf` | the two papers rebuilt with the WP_draft2 conventions (same captions, short notes with a separate Source line, same main/appendix split) and plainer text; these supersede the 2026-09-22 PDFs below |
| 2a''' | `docs/WP_paper_2026-09-22.pdf` | **THE working paper** as it goes to Christian: title page with abstract, Introduction and Data written out in plain prose, the `[Main text]` exhibits of the draft in stylized-fact order with a short reading paragraph each, the `[Appendix]` ones in the appendix, and Literature / Theoretical framework / Conclusions stated but not developed. The US story is the through-line. Overleaf source in `output/wp/overleaf_WP_paper*`; `python wp_build_overleaf.py paper` |
| 2a'''' | `docs/WP_paperagro_2026-09-22.pdf` | **the agriculture-only companion** (HS 01--24, US\$78.9bn/yr): the same six facts and the same data section computed within agriculture, plus an "Inside agriculture" section (SITC divisions, HS sections, BEC end use, agro inputs, top HS6). Deliberately low-profile on the United States: no US section, no three-share exhibits, no "who carries the exports to the USA", no US home-products table; the USA appears only as one parent country among others in the rankings, and the parent x destination tables sit in the appendix. Everything is region-wide. `python wp_build_overleaf.py paperagro` |
| — | both of the above need `python wp_paper_sample.py` (writes `tab_paper_sample*` for scopes `all` and `agro`) if the sample tables are missing | |
